...
class CalculatorApp(QtWidgets.QMainWindow):
    ...
    def formula_processing(self, formula: str) -> None:
        ...
        if self.antivirus(formula_st):  # Проверка на допустимые символы
            print("*", formula_st)  # Логирование стандартизированной формулы
            self.out_result(formula, self.calculation(formula_st))  # Вывод результата
        else:
            print("**", formula_st)  # Логирование недопустимой формулы
            self.out_result(formula, "Недопустимый символ")  # Вывод сообщения об ошибке
    ...
    def out_in_tbl_results(self, formula: str, result: str):
        ...
        # Добавление кнопки в новую строку
        self.add_button_in_row(new_row)

        # Добавление формулы в новую строку
        self.add_item_in_row(new_row, 1, formula, "left")

        # Добавление результата в новую строку и выравнивание по правому краю
        self.add_item_in_row(new_row, 2, result, "right")

        # Прокрутка таблицы до последнего результата
        self.tblResults.scrollToBottom()

    def on_button_copy_click(self, row: int) -> None:
    ...
    def add_item_in_row(self, row: int, column: int, text: str, align: str = "left"):
        ...
        установка флага редактирования и выравнивание текста."""
    ...
    def add_button_in_row(self, row: int) -> None:
        ...
        button.clicked.connect(lambda checked, r=row: self.on_button_copy_click(r))

        # Добавление кнопки в соответствующую ячейку таблицы
    ...
    def symbol_standardization(formula: str) -> str:
        """Замена нестандартных символов на стандартные."""
    ...
    def calculation(formula: str) -> str:
        ...
        try:
            with redirect_stderr(NullIO()):  # Подавление вывода ошибок
...
if __name__ == "__main__":
    ...
    calc_app.start()  # Запуск приложения
